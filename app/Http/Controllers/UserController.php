...
class UserController extends Controller
{
    ...
    public function cekkode(Request $request)
    {
        ...
        // Cari kode promo yang masih aktif dan belum expired
        $cekkode = KodePotongan::where('kode', $kode)
            ->where('status', 1)
        ...
        // Cek batas pemakaian kode promo
        $cektransaksi = Transaksi::where('fk_promo_id', $cekkode->id)->where('status', 1)->count();
        if ($cektransaksi >= $cekkode->batas) {
            return response()->json([
                'status' => false,
                'message' => 'Kode voucher sudah habis'
            ]);
        }

        // Cek apakah user sudah menggunakan kode ini
        $cektransaksiuser = Transaksi::where('fk_promo_id', $cekkode->id)
        ...
        // Hitung potongan harga berdasarkan tipe (1 = Harga, 2 = Persen)
        $potongan = ($cekkode->tipe == 2)
            ? ($paket->harga * $cekkode->jumlah / 100)
            : $cekkode->jumlah;

        $total = max($paket->harga - $potongan, 20000);
    ...
    //     if ($cekkode) {
    //         $cektransaksi = Transaksi::where('fk_promo_id', '=', $cekkode->id)->where('status', 1)->get();

    //         if (count($cektransaksi) < $cekkode->batas) {
    //             $cektransaksiuser = Transaksi::where('fk_promo_id', '=', $cekkode->id)->where('fk_user_id', Auth::id())->first();
    ...
    public function selesaiujiankecermatan(Request $request, $id = NULL)
    {
        ...
        if ($ret) {
            ...
            if (!$id) $id = $request->idsoalmst;
        }

        if (is_string($id) && !ctype_digit($id)) $id = Crypt::decrypt($id);
    ...
    public function updatejawaban(Request $request)
    {
        ...
        if ($cekismengerjakan->jenis_penilaian != 2) {
    ...
    public function selesaiujian(Request $request, $id = NULL)
    {
        ...
        if ($dataupaketsoalmst->jenis_penilaian != 2) {

            $pointkecermatan  = 0;
            $maxpointkecermatan  = 0;
            ...
            foreach ($upaketsoalktg as $key) {
                ...
                if ($key->fk_u_paket_soal_kecermatan_mst) {
                    ...
                    $pointkecermatan += $jumlahbenar;
                    $maxpointkecermatan += $jumlahsoal;
            ...
            $pointtotalbenar = $totalbenar->sum('benar_salah') + $pointkecermatan;

            $pointmax = $totalbenar->sum('max_point') + $maxpointkecermatan;
            ...
            $datanilaiakhir['set_nilai'] = $totalbenar->sum('benar_salah') + $pointkecermatan;

            $datanilaiakhir['point'] = $totalbenar->sum('benar_salah') + $pointkecermatan;
...

    {
        ...
        if ($perpaket = $request->perpaket) {

            $update = UPaketSoalKtg::where('fk_u_paket_soal_mst', $id)->where('fk_u_paket_soal_kecermatan_mst', '=', 0)->update($updateselesai);
        } else {
    ...
    public function detailhasil($id)
    {
        ...
        if ($upaketsoalmst->is_mengerjakan == 1) {
            ...
            if ($upaketsoalmst->fk_u_paket_soal_kecermatan_mst) {
                $soalmst = UPaketSoalKecermatanSoalMst::where('fk_u_paket_soal_kecermatan_mst', $upaketsoalmst->fk_u_paket_soal_kecermatan_mst)->get();
            } else {
                $soalmst = [];
            }
