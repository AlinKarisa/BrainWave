...

{
...

    {
        ...
        $kodepotongan = KodePotongan::find($id);

        $menu = 'master';

        $submenu='kodepotongan';

        $pengguna = Transaksi::where('fk_promo_id','=',$id)->where('status',1)->get();

        $data_param = [

            'menu','submenu','pengguna','kodepotongan'

        ];

        return view('master/kodepotonganpengguna')->with(compact($data_param));
