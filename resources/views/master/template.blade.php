                  ...
                  <td width="1%" class="_align_center">
                    <div class="btn-group">
                      <span data-toggle="tooltip" data-placement="left" title="Lihat Logo Besar">
                        <button onclick="modalImage('Logo Besar','{{asset($key->logo_besar)}}')"
                          class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                      </span>
                      <span data-toggle="tooltip" data-placement="left" title="Lihat Logo Kecil">
                        <button onclick="modalImage('Logo Kecil','{{asset($key->logo_kecil)}}')"
                          class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                      </span>
                      <span data-toggle="tooltip" data-placement="left" title="Lihat Banner 1">
                        <button onclick="modalImage('Banner 1','{{asset($key->Banner1)}}')"
                          class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                      </span>
                      <span data-toggle="tooltip" data-placement="left" title="Lihat Banner 1">
                        <button onclick="modalImage('Banner 2','{{asset($key->Banner2)}}')"
                          class="btn btn-sm btn-outline-success"><i class="fas fa-eye"></i></button>
                      </span>
                      <span data-toggle="tooltip" data-placement="left" title="Ubah Data">
                        <button data-toggle="modal" data-target="#modal-edit-{{$key->id}}" type="button"
      ...
      <form method="post" id="formData_{{$key->id}}" class="form-horizontal">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="nama_{{$key->id}}" name="nama[]" placeholder="Nama Website"
              value="{{$key->nama}}">
          </div>
          <div class="form-group">
            <label for="email_{{$key->id}}">Email<span class="bintang">*</span></label>
            <input type="text" class="form-control" id="email_{{$key->id}}" name="email[]" placeholder="Email"
              value="{{$key->email}}">
          </div>
          <div class="form-group">
            <label for="no_hp_{{$key->id}}">No.Hp<span class="bintang">*</span></label>
            <input type="text" class="form-control int" id="no_hp_{{$key->id}}" name="no_hp[]" placeholder="No.Hp"
              value="{{$key->no_hp}}">
          </div>
          <div class="form-group">
            ...
            <select class="form-control form-control-lg _select2" id="fk_provinsi" name="fk_provinsi">
              ...
              <option value="{{$keyprov['province_id']}}" {{$key->fk_provinsi==$keyprov['province_id'] ? "selected" :
                ""}}>{{$keyprov['province']}}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            ...
            <select class="form-control form-control-lg _select2" id="fk_kabupaten" name="fk_kabupaten">
              ...
              <option value="{{$keykab['city_id']}}" {{$keykab["city_id"]==$key->fk_kabupaten ? "selected" :
                ""}}>{{$keykab['type']." ".$keykab['city_name']}}</option>
              @endforeach
              @endif
            </select>
          ...
          <div class="form-group">
            <label for="copyright_{{$key->id}}">Copyright<span class="bintang">*</span></label>
            <textarea name="copyright[]" id="copyright_{{$key->id}}" rows="5" class="form-control"
              placeholder="Copyright">{{$key->copyright}}</textarea>
          </div>
          <!-- <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              <div class="custom-file">
                <input type="file" class="custom-file-input input-foto" idlabel="label-logo_besar-{{$key->id}}"
                  name="logo_besar[]" id="logo_besar-{{$key->id}}">
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              <div class="custom-file">
                <input type="file" class="custom-file-input input-foto" idlabel="label-logo-kecil-{{$key->id}}"
                  name="logo_kecil[]" id="logo-kecil-{{$key->id}}">
          ...
          <div class="form-group">
            <label for="banner1">Banner 1 <span class="input-keterangan">(jpg/jpeg/png)</span></label>
            <div class="input-group">
              <div class="custom-file">
                <input type="file" class="custom-file-input input-foto" idlabel="label-banner1-{{$key->id}}"
                  name="banner1[]" id="banner1-{{$key->id}}">
                <label id="label-banner1-{{$key->id}}" class="custom-file-label" for="banner1">Pilih File</label>
              </div>
            </div>
          </div>

          <div class="form-group">
            <label for="banner2">Banner 2 <span class="input-keterangan">(jpg/jpeg/png)</span></label>
            <div class="input-group">
              <div class="custom-file">
                <input type="file" class="custom-file-input input-foto" idlabel="label-logo-kecil-{{$key->id}}"
                  name="banner2[]" id="banner2-{{$key->id}}">
                <label id="label-banner2-{{$key->id}}" class="custom-file-label" for="banner2">Pilih File</label>
              </div>
            </div>
          </div>
