...
<script type="text/javascript">
 ...
 function ganti_soal() {
    ...
    // If we exceed the available questions, return to stop progression
    if (nosoal >= soals.length) return;

    var id = soals[nosoal];
    
    // Check for any unanswered pills within this question
    var hasUnansweredPills = false;
    $('#pills-' + id + ' ._radio').each(function() {
        if (!$(this).is(':checked')) {
            hasUnansweredPills = true;
            return false; // Break loop if unanswered pill found
        }
    });

    // If unanswered pills exist in the previous question, keep displaying the first unanswered pill
    if (hasUnansweredPills) {
        // Find the first unanswered pill in the current question and set it to active
        $('#pills-' + id + ' ._radio').each(function() {
            if (!$(this).is(':checked')) {
                $(this).closest('.tab-pane').addClass('show active');
                return false; // Break loop once first unanswered pill is activated
            }
        });
    } else {
