                ...
                <tbody>
                  @if(count($udatapaket)>0)
                    ...
                    <tr>
                      ...
                      <td width="40%">{{ optional($key->user_r)->name }} @if($loop->iteration == 1)<i style="color:gold" class="ti-medall btn-icon-prepend"></i>@endif</td>
                      <td width="10%" style="text-align:left" class="font-weight-bold">{{$key->set_nilai}}</td>
                      <td style="text-align:left" width="40%">
    <span data-bs-toggle="tooltip" 
          data-bs-placement="top" 
          title="Kecamatan : {{ $key->user_r?->kecamatan_r?->nama ?: 'Kecamatan tidak tersedia' }}, 
                 {{ $key->user_r?->kabupaten_r ? ucwords(strtolower($key->user_r?->kabupaten_r?->nama)) : 'Kabupaten tidak tersedia' }}, 
                 Provinsi : {{ $key->user_r?->provinsi_r?->nama ?: 'Provinsi tidak tersedia' }}">
        {{ $key->user_r?->kabupaten_r ? ucwords(strtolower($key->user_r?->kabupaten_r?->nama)) : 'Kabupaten tidak tersedia' }}, 
        {{ $key->user_r?->provinsi_r?->nama ?: 'Provinsi tidak tersedia' }}
    </span>
</td>
